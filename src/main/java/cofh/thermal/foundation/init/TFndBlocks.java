...
public class TFndBlocks {
    ...
    //                () -> new BlockItemCoFH(BLOCKS.get(ID_OIL_SAND), new Item.Properties()).setBurnTime(2400).setShowInGroups(getFlag(FLAG_RESOURCE_OIL)).setModId(ID_THERMAL_FOUNDATION));
    //
    //        registerBlockAndItem(ID_OIL_RED_SAND, () -> new SandBlock(11098145, copy(Blocks.RED_SAND)),
    //                () -> new BlockItemCoFH(BLOCKS.get(ID_OIL_RED_SAND), new Item.Properties()).setBurnTime(2400).setShowInGroups(getFlag(FLAG_RESOURCE_OIL)).setModId(ID_THERMAL_FOUNDATION));
    //    }
    //
    //    private static void registerStorage() {
    //
    //        registerBlock(raw(ID_TIN_BLOCK), () -> new Block(of().mapColor(TERRACOTTA_LIGHT_BLUE).strength(5.0F, 6.0F).requiresCorrectToolForDrops()), ID_THERMAL_FOUNDATION);
    //        registerBlock(raw(ID_LEAD_BLOCK), () -> new Block(of().mapColor(TERRACOTTA_BLUE).strength(5.0F, 6.0F).requiresCorrectToolForDrops()), ID_THERMAL_FOUNDATION);
    //        registerBlock(raw(ID_SILVER_BLOCK), () -> new Block(of().mapColor(TERRACOTTA_LIGHT_GRAY).strength(5.0F, 6.0F).requiresCorrectToolForDrops()), ID_THERMAL_FOUNDATION);
    //        registerBlock(raw(ID_NICKEL_BLOCK), () -> new Block(of().mapColor(TERRACOTTA_YELLOW).strength(5.0F, 6.0F).requiresCorrectToolForDrops()), ID_THERMAL_FOUNDATION);
    //
    ...
    //        registerBlock(ID_RUBBERWOOD_WOOD, () -> new RotatedPillarBlock(of().mapColor(QUARTZ).strength(2.0F).sound(SoundType.WOOD)), ID_THERMAL_FOUNDATION);
    //        registerBlock(ID_STRIPPED_RUBBERWOOD_WOOD, () -> new RotatedPillarBlock(of().mapColor(TERRACOTTA_GREEN).strength(2.0F).sound(SoundType.WOOD)), ID_THERMAL_FOUNDATION);
    //        registerBlock(ID_RUBBERWOOD_SAPLING, () -> new SaplingBlock(new RubberTreeGrower(), of().mapColor(MapColor.PLANT).noCollission().randomTicks().instabreak().sound(SoundType.GRASS)), ID_THERMAL_FOUNDATION);
    ...
    //
    //        registerBlockOnly("rubberwood_sign", () -> new StandingSignBlock(of().noCollission().strength(1.0F).sound(SoundType.WOOD), RUBBERWOOD));
    //        registerBlockOnly("rubberwood_wall_sign", () -> new WallSignBlock(of().noCollission().strength(1.0F).sound(SoundType.WOOD).lootFrom(BLOCKS.getSup("rubberwood_sign")), RUBBERWOOD));
    //    }
    //    // endregion
    //
    //    // region HELPERS
