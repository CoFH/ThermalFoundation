...
public class UnderwaterMinecartRenderer<T extends AbstractMinecartEntity> extends EntityRenderer<T> {
    ...
    public void render(T entityIn, float entityYaw, float partialTicks, MatrixStack poseStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
        ...
        if (vec3d != null) {
            ...
            poseStackIn.translate(vec3d.x - d0, (vec3d1.y + vec3d2.y) / 2.0D - d1, vec3d.z - d2);
        ...
        this.modelMinecart.renderToBuffer(poseStackIn, ivertexbuilder, packedLightIn, OverlayTexture.NO_OVERLAY, 1.0F, 1.0F, 1.0F, 1.0F);
        poseStackIn.popPose();
    }
    ...
    protected void renderBlockState(T entityIn, float partialTicks, BlockState stateIn, MatrixStack poseStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
